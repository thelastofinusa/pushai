 ...
 */
export async function getStoredConfig(): Promise<SetupConfig | null> {
  const configPath = getConfigPath();

  const fileConfig = readJsonFile<Partial<SetupConfig>>(configPath);

  if (!fileConfig?.mode || !fileConfig.model) return null;

  if (fileConfig.mode === "byok") {
 ...
 */
export async function setStoredConfig(data: SetupConfig): Promise<void> {
  ensureConfigDir();

  const configPath = getConfigPath();

  // Cast to a generic record so TypeScript allows us to extract apiKey safely
  // regardless of which 'mode' the data is in.
  const { apiKey, ...rest } = data as Record<string, unknown>;

  // Store API key separately if it exists
